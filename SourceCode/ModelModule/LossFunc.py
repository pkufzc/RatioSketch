"""Loss functions used for training the decoder.

AutoWeightLoss implements learnable weights for multiple loss terms.
RatioLoss combines a heap-focused ratio error and an item-size error.
"""
...
class AutoWeightLoss(nn.Module):
    """Automatically weight multiple loss terms with learnable params."""
...
class RatioLoss(nn.Module):
    """Loss used for training the decoder.

    - compares predicted ratios vs. true ratios for heap (high-frequency) items
    - penalizes error in predicted item_size (dec_is)
    """
    ...
    def print_real_ratio_max(self, real_ratio, confidence=0.95):
        ...
        print(f"real_ratio {confidence*100:.1f}% CI max: {max_in_ci:.6f}")
    ...
    def forward_dec(self, batch_pred, batch_read_head, heap_flags, batch_y, dec_is):
        ...
        # heap portion: weighted absolute error on ratio for heap items
        ratio_loss_heap = 0
        ...
        # item size loss: difference between predicted and real item counts
        real_is = (
