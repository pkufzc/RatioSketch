...
class TCMS:
    ...
    def update_batch(self, x_batch, c_batch):
        x_np = np.asarray(x_batch.cpu() if hasattr(x_batch, 'cpu') else x_batch, dtype=np.int64).reshape(-1)
        c_np = np.asarray(c_batch.cpu() if hasattr(c_batch, 'cpu') else c_batch, dtype=np.int32).reshape(-1)
