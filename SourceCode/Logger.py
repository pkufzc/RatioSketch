...
class Logger:
    ...
    def _eval_ratio(self, ratio_model, taskdata, mem_size, device_list, model_name):
        ...
        # If evaluating, optionally load the corresponding pretrained model
        # (the loading steps are left commented out as an example).
        if model_name == "CMS+RS":
            # model_path = os.path.join(logdir, f"CMS/10kb_model")
            ...
            # model_path = os.path.join(logdir, f"TCMS/10kb_model")
            # state_dict = torch.load(model_path)
            # ratio_model.load_state_dict(state_dict, strict=False)
            # del state_dict
        ...
        for device_type in device_list:
            ...
            ratio_pred, *_ = ratio_model.dec_query(items, freqs_sum)
    ...
    def _eval_sketch(self, mem_size, taskdata, model_class, model_name, device_list):
        ...
        for device_type in device_list:
            ...
            model.update_batch(items, freqs)
